...
$result=mysqli_query($conn,"select* from testimonial  where 	ID='$ss' ");
while($row=mysqli_fetch_array($result))
{
echo $row['address'];
}
...
?> 
 <article class="post clearfix mb-0">
                <div class="entry-header">
                  <div class="post-thumb thumb"> <img  src="testimonial/<? echo $row['photo']; ?>"   alt="" class="img-responsive img-fullwidth"> </div>
                </div>
                  ...
                  <div class="entry-meta media no-bg no-border mt-15 pb-20">
                    ...
                    <div class="media-body pl-15">
                      <div class="event-content pull-left flip">
                        <h3 class="entry-title text-white text-uppercase pt-0 mt-0"><a href="news.php?ID=<? echo $row['ID']; ?>"><? echo   $row['address'] ; ?> </a></h3>
                        <span class="mb-10 text-gray-darkgray mr-10 font-13"><i class="fa fa-commenting-o mr-5 text-theme-colored"></i> 214 تعليق</span>                       
                        <span class="mb-10 text-gray-darkgray mr-10 font-13"><i class="fa fa-heart-o mr-5 text-theme-colored"></i> 895 اعجاب</span>
                      </div>
                    </div>
                  </div>
                  <p class="mb-15" align="justify"><?php echo $row['news']; ?></p>
               <? if($row['sorc']!="") { ?>   <p><b><font color="#800000">المصدر :</font></b> <?  echo $row['sorc']; ?></a></p> <? } ?>
 
 
                  <div class="mt-30 mb-0">
                    <h5 class="pull-left flip mt-10 mr-20 text-theme-colored">مشاركة الخبر : </h5>
      ...
      <? }} ?>         
          ...
          <div class="col-md-3">
            <div class="sidebar sidebar-right mt-sm-30">
              <div class="widget">
                <h5 class="widget-title line-bottom">البحث عن الآحداث والفعاليات</h5>
                <div class="search-form">
                  <form>
                    <div class="input-group">
                      <input type="text" placeholder="البحث" class="form-control search-input">
              ...
              <div class="widget">
                <h5 class="widget-title line-bottom">تصنيف الآحداث والفعاليات</h5>
                <div class="categories">
                  <ul class="list list-border angle-double-right">
                    <?php
$C=0;
$result=mysqli_query($conn,"select DISTINCT year from testimonial ORDER BY year DESC  ");
while($row=mysqli_fetch_array($result))
{
$C++;
?> <li><a  href="all_testimonial.php?year=<? echo $row['year']; ?>" title="<? echo $row['year']; ?>">الآحداث والفعاليات لعام <? echo $row['year']; ?></a></li>
<? } ?>
                  </ul>
                </div>
              </div>
              <div class="widget">
                <h5 class="widget-title line-bottom">  اخر الآحداث والفعاليات</h5>
                <div class="latest-posts">
<?php
$C=0;
$result=mysqli_query($conn,"select * from testimonial where photo!='-1.' ORDER BY ID DESC limit 5");
while($row=mysqli_fetch_array($result))
{
$C++;
?>                    
                  <article class="post media-post clearfix pb-0 mb-10">
                    <a class="post-thumb" href="#"><img src="testimonial/<? echo $row['photo']; ?>"  style="width:75px;height:50px"   alt=""></a>
                    <div class="post-right">
                      <h5 class="post-title mt-0"><a href="testimonial.php?ID=<? echo $row['ID']; ?>"><? echo $row['address']; ?></a></h5>
