...
$query = "select * from user_crm,user_login 
where user_crm.username=user_login.username  and date>='$_POST[date_from]' and date<='$_POST[date_to]'  order by date,user_crm.username DESC"; }
 else {
$query = "select * from user_crm,user_login 
where user_crm.username=user_login.username  and date>='$_POST[date_from]' and date<='$_POST[date_to]'  and user_crm.ID='$_POST[Employee_ID]'   order by date,user_crm.username DESC";
...
while($row1=mysqli_fetch_array($result1))
{
  echo $row1['Branches_name'];  } } ?></td> 
                         <td align="right"><p align="center"><?php echo $row['date']; ?></td> 
