...
where user_cors.cors_ID=reg_cors.no   and cors.cors_ID=subjects.cors_ID and address=cors.ID  and Supervisor='6' and cors.ID='$ss'";
elseif($_SESSION['acount_type']==2)
$query = "select * from reg_cors,user_cors,subjects,cors 
where user_cors.cors_ID=reg_cors.no and user_ID='$_SESSION[user_ID]' and cors.cors_ID=subjects.cors_ID and address=cors.ID  and Supervisor='6' and cors.ID='$ss'";
elseif($_SESSION['acount_type']==3)
$query = "select * from user_crm,reg_cors,user_cors,subjects,cors
where user_cors.cors_ID=reg_cors.no and user_crmBranches='$_SESSION[Branches]' and user_crm.ID=user_cors.user_ID and cors.cors_ID=subjects.cors_ID and address=cors.ID  and Supervisor='6' and cors.ID='$ss'";
...
<tr> 
 ...
 <?php if( $row['reg_ok']!=1) { ?><input  required="required"   onclick="return confirm('هل تريد حفظ التغيرات ؟')"  type="submit" value="تأكيد" name="B22" style="float: left" ><?php } ?>
 </td>
...
if($row['status']=="paid") echo '<font color="green">عملية الدفع مقبولة</font>';  ?> </td>
...
echo $names=$row1['name'];
 ...
 ?>
											...
											<td align="center"   > <?php if($row['Supervisor']==0) echo '<font color=#00ffdd><i class="fa fa-circle fa-2x" aria-hidden="true" ></i></font>'; ?>
						   ...
						   <?php if($row['Supervisor']==6) echo '<font color=red><i class="fa fa-money fa-2x" aria-hidden="true" ></i></font>'; ?>
											...
											<td align="center"   ><?php echo $row['reg_price']; ?></td>
