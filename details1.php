...
if (isset($_POST['contact'])) {
  ...
  if ($responseKeys["success"]) {
    ...
    // echo '<pre>';
    // var_dump($_REQUEST["sponsor"]);
    // exit;

    $sponsor = $_REQUEST["sponsor"];
    $come_from = $_REQUEST["social"];

    $sql = "INSERT INTO reg_cors (name, mobile, eqt, dates, address, mail, iqama, nat, qualification, reg_price,sponsor,come_from) 
    VALUES ('$name', '$phone', '$message1', CURDATE(), '$cors', '$email', '$iqama', '$nat', '$qualification', '$reg_price','$sponsor' , '$come_from')";
    if (mysqli_query($conn, $sql)) {
      ...
      $insert_user_cors_sql = "INSERT INTO user_cors (`cors_ID`, `user_ID` , `time`, `date`, `come_from`) 
                       VALUES ('$last_id', '$sponsor', '$time', CURDATE(), '$social')";
      mysqli_query($conn, $insert_user_cors_sql);
    }

    $update_sql = "UPDATE reg_cors 
           SET name='$name', mobile='$phone', eqt='$message1', dates=CURDATE(), address='$cors', mail='$email', iqama='$iqama', 
               nat='$nat', qualification='$qualification', reg_price='$reg_price' 
           WHERE iqama='$iqama' AND address='$cors'";
...
<?php
    ...
    $query11 = "SELECT count(user_ID)+cou, ID FROM user_crm 
        LEFT JOIN user_cors ON user_crm.ID=user_cors.user_ID 
        WHERE acount_type=2 AND Activation=1 AND stop=0 AND ID IN (SELECT user_ID FROM user_course WHERE cors_ID='$cors_ID1') 
        GROUP BY ID ORDER BY count(user_ID)+cou ASC LIMIT 1";
