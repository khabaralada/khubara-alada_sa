...
<head>
<meta charset="UTF-8">
<title>الوسائل التدريبية المستخدمة في البرنامج التدريبي</title>
...
<h1><small>الوسائل التدريبية المستخدمة في البرنامج التدريبي</small></h1>
</section>
...
<label>الوسائل التدريبية المستخدمة في البرنامج التدريبي</label>
...
<table    class="table table-bordered table-striped"  >
                    <thead>
                      <tr>
                        ...
                        <th align="right"><p align="center">الوسائل التدريبية المستخدمة في البرنامج التدريبي</th>
                        <th align="right"><p align="center"></th>
                      </tr>
                    </thead>
                    <tbody>
                     <?php
include ("conn.php");
$query = "select * from tools where cors_ID='$_GET[cors_ID]' ";
...
?>
 <tr>
                        ...
                        <td align="right"><p align="center"><?php echo $row['tools_ID']; ?></td>
                        <td align="right"><p align="center"><?php echo $row['tools']; ?></td>
 
 
						
                        <td align="center">
                        <a href="" data-id="<?php echo $row['tools_ID']; ?>" data-target="#myModaEdit<?php echo $row['tools_ID']; ?>" data-toggle="modal" class="btn btn-info btn-sm   "><i class="fa fa-pencil"></i> </a>
                        <a href="" data-id="<?php echo $row['tools_ID']; ?>" data-target="#myModaDelete<?php echo $row['tools_ID']; ?>" data-toggle="modal" class="btn btn-danger btn-sm  "><i class="fa fa-trash-o "></i></a>
</td>
                      	</form>
                      </tr><div class="example-modal">
            <div class="modal" id="myModaEdit<?php echo $row['tools_ID']; ?>" role="dialog">
              <div class="modal-dialog">
                <div class="modal-content">                <form action="" method="post">
                  ...
                  <div class="modal-body">
					...
					<input  type=hidden name="idds"   size="20"   value="<?php echo $row['tools_ID']; ?>" required="required"   class="form-control"  >
...
<div class="form-group col-lg-12">
<label>الوسائل التدريبية المستخدمة في البرنامج التدريبي</label>
<input  name="T1"   size="20"   required="required"   class="form-control" value="<?php echo $row['tools']; ?>"  >
</div>  
...
<input type="submit" class="btn btn-success"  value="حفظ التغييرات" name="edit"> 
 ...
 <div class="example-modal">
            <div class="modal" id="myModaDelete<?php echo $row['tools_ID']; ?>" role="dialog">
              <div class="modal-dialog">
                <div class="modal-content">
               <form action="" method="post">
                  <div class="modal-header">
					<b><font face="Droid Arabic Kufi" size="5">حذف بيانات : <?php echo $row['name']; ?></font><font size="5"></font></b>
					<input  type=hidden name="idd"   size="20"   value="<?php echo $row['tools_ID']; ?>" required="required"   class="form-control"  >
...
$sql="insert into tools set tools='$T1',cors_ID='$cors_ID'  ";
if(mysqli_query($conn,$sql))
{
...
$sql="update tools set tools='$T1'  where tools_ID='$idds'";
if(mysqli_query($conn,$sql))
{
$last_id = mysqli_insert_id($conn);
echo '<script>
...
$sql="delete from tools where tools_ID='$idd'";
if(mysqli_query($conn,$sql))
{
$last_id = mysqli_insert_id($conn);
echo '<script>
