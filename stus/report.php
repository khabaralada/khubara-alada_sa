...
if($ID==1) echo 'الدورات التدريبية';
if($ID==2) echo 'الدورات المنتهية';
if($ID==3) echo 'الدورات القائمة';
if($ID==4) echo 'المتدربين';
if($ID==5) echo ' المتدربين المنتهي دوراتهم';
if($ID==6) echo 'المتدربين القائم دوراتهم';
if($ID==7) echo '';
if($ID==8) echo '';
...
include ("conn.php");
  $bre= mysqli_real_escape_string($conn,htmlspecialchars(intval($_GET['bre']), ENT_QUOTES, "utf-8")); 
  $cor= mysqli_real_escape_string($conn,htmlspecialchars(intval($_GET['cor']), ENT_QUOTES, "utf-8")); 
  $ID= mysqli_real_escape_string($conn,htmlspecialchars(intval($_GET['ID']), ENT_QUOTES, "utf-8")); 

 if($bre!='')                $xx=' and cors.Branches='.$bre; else $xx='';
 if($cor!='')               $xx1=' and cors.cors_type='.$cor; else $xx1='';

if($ID==1)
$query = "select * from cors,subjects,teacher,branches   where branches.Branches_ID=cors.Branches and  teacher.teacher_id=cors.traning_name and subjects.cors_ID=cors.cors_ID  $xx $xx1 order by cors_no DESC";
 if($ID==2)
$query = "select * from cors,subjects,teacher,branches   where branches.Branches_ID=cors.Branches and  teacher.teacher_id=cors.traning_name and subjects.cors_ID=cors.cors_ID  $xx $xx1 order by cors_no DESC";
